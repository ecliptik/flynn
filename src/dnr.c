...
/* Retro68 compatibility */
#define PBHGetFInfo(pb, async) ((async) ? PBHGetFInfoAsync((HParmBlkPtr)(pb)) : PBHGetFInfoSync((HParmBlkPtr)(pb)))
#define CloseWD(refnum) /* no-op, working directories auto-close */
...
OpenResolver(char *fileName)
{
	...
	ReserveMem(16000L); 			/* s-dorner@uiuc.edu, 9/5/91 */
...
DNSResolveName(char *name, unsigned long *ipAddress, void (*yielder)(void))
{
	...
	memset(&aHostInfo, 0, sizeof(aHostInfo));
	...
	if (osErr == cacheFault) {
		...
		while (!done) {
			/* Timeout after 30 seconds (1800 ticks) */
			if (TickCount() - start_ticks > 1800L) {
				osErr = authNameErr;
				return osErr;
			}
			if (yielder)
